...
async function sendmesg(client, interactionCache, message) {
  ...
  if (hasKeysExclusively(message, ["plat", "user", "priv", "mesg"])) {
    ...
    hasKeysExclusively(message, ["plat", "fief", "chan", "mrid", "phem","mesg"])
  ) {
    const guilds = await client.guilds.fetch();
    let guild = guilds.find(g => g.name === fief);
    if (guild) { guild = await guild.fetch() }
    ...
    if (mrid.startsWith("interaction:")) {
      ...
      if (interaction.replied) {
        interaction.followUp({ content: mesg, ephemeral: phem });
      } else {
        interaction.reply(   { content: mesg, ephemeral: phem });
      }
    } else {
      if (channel) await channel.messages.fetch(mrid).then(m => m.reply(mesg));
    }
